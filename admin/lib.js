...
var Validar = {
    ...
    numeroDecimal : function(evento,cadena,separador){
    	var tecla = (evento.which) ? evento.which : evento.keyCode;
        var numero = cadena.split(".");

    	if ( cadena === '' ) { // NO INGRESAR AL PRINCIPIO . SI CUANDO TENGA UN NUMERO PRIMERO
    		if ( (tecla >= 48 && tecla <= 57) ) {	    		
	            return true;
	        }
	        return false;
        ...
        if ( cadena === '0' ) { // VALIDAR SI EL PRIMERO ES CERO
            if ( tecla == 46 ) {
                return true;
            }
            return false;
        ...
        if ( (tecla >= 48 && tecla <= 57) || ( tecla == 46 ) ) {            
            if ( (cadena.indexOf('.') > 0) && (tecla == 46) ) { // SOLO UN PUNTO
                return false;
            }

            if ( numero[0].length+separador == (cadena.length-1) ) { // SEPARADOR DE DECIMALES
                return false;
            }
            return true;
        }
